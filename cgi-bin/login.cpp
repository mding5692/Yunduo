...
int main() {
	// CGICC object and loginServer objected to call their functions
	Cgicc formData;
	loginServer login;

	// Necessary boilerplate to display an html code in C++
	...
	// Grabs the username and password using CGICC class functions
	form_iterator username = formData.getElement("username");
	form_iterator password = formData.getElement("password");

	// Authenticates and checks permissions of the user
	cout << "<br>";
	int pmt;
	pmt = login.checkPassword(**username,**password);
		
	// Depending on permission of user, display the details they can see	
	switch(pmt){
		case 5: {
		...
		case 1: {
		cout << "<a href='../upLoad.html' id='recommend-btn' class='main-nav-btn' onclick=""> <i class='fa fa-arrow-circle-up'></i> Upload file </a> <a href='../cgi-bin/downloadnodelete.cgi' id='recommend-btn' class='main-nav-btn' onclick=""> <i class='fa fa-arrow-circle-down'></i> Download file </a>" << endl;
		break;
		}
		case 0: {
		cout << "<a href='../cgi-bin/downloadnodelete.cgi' id='recommend-btn' class='main-nav-btn' onclick=""> <i class='fa fa-arrow-circle-down'></i> Download file </a>" << endl;
		break;
		}
		case 3: break;
		case 4: break;
	...
	// If username is wrong or password is right, show folder contents, else hide folder contents link
	if (pmt != 3 && pmt != 4) {
